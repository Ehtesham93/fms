    ...
    try {
      ...
      for (const nodeMeta of mastersMeta) {
        try {
          ...
          this.logger.warn(`Redis node error for key ${key}:`, nodeError.message);
        }
      }
        ...
        if (value === null) {
          ...
          for (const nodeMeta of mastersMeta) {
            try {
              ...
              this.logger.warn(`Redis node error for key ${keys[i]}:`, nodeError.message);
            }
          }
        }
        result[keys[i]] =value;
        ...
        if (setError.message && setError.message.includes('MOVED')) {
          ...
          if (movedMatch) {
            ...
            try {
              ...
              this.logger.error(`Redis set error on target node ${host}:${port}:`, nodeError);
