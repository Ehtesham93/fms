export default class MetaSvcDB {
  ...
  // Vehicle City CRUD
  async createVehicleCity(citycode, cityname) {
    try {
      // Check if citycode already exists
      const existingCity = await this.isCityCodeAvailable(citycode);
      if (!existingCity.isavailable) {
        const error = new Error("City code already exists");
        error.errcode = "CITY_CODE_ALREADY_EXISTS";
        error.errdata = {
          citycode: citycode,
        };
        throw error;
  ...
  // Vehicle Dealer CRUD
  async createVehicleDealer(dealercode, dealername) {
    try {
      ...
      if (!existingDealer.isavailable) {
        const error = new Error("Dealer code already exists");
        error.errcode = "DEALER_CODE_ALREADY_EXISTS";
        error.errdata = {
  ...
  // Vehicle Color CRUD
  async createVehicleColor(colorcode, colorname) {
    try {
      ...
      if (!existingColor.isavailable) {
        const error = new Error("Color code already exists");
        error.errcode = "COLOR_CODE_ALREADY_EXISTS";
        error.errdata = {
          colorcode: colorcode,
        };
        throw error;
      ...
      let query = `
        ...
        `;
      let result = await this.pgPoolI.Query(
        query,
        [colorcode.toUpperCase(), colorname.toUpperCase()]
      );
