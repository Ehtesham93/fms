...
async function flushMetrics() {
  if (globalState.pendingMetrics.length === 0) return;

  try {
    const metricsToWrite = globalState.pendingMetrics
      .map((metric) => {
  ...
  const histogramObj = {
    observe: (value, labels = {}) => {
      ...
      const metricData = {
        ...
        avg: observations.reduce((a, b) => a + b, 0) / observations.length,
      };
  ...
  const summaryObj = {
    observe: (value, labels = {}) => {
      ...
      const metricData = {
        ...
        count: observations.length,
        sum: observations.reduce((a, b) => a + b, 0),
        ...quantiles.reduce((acc, q) => {
          acc[`quantile_${Math.floor(q * 100)}`] = getQuantile(sorted, q);
          return acc;
        }, {}),
      };
      writeMetric(metricData);
